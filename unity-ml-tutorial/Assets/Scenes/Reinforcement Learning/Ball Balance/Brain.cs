...
namespace BallBalance
{
    public class Brain : MonoBehaviour
    {
        ...
        int mCapacity = 10000;                              // memory capcity
        ...
        Vector3 ballStartPos;                               // record start position of the ball
        int failCount = 0;                                  // count when the ball is dropped
        float tiltSpeed = 0.5f;                             // max angle to apply to tilting each update
                                                            // make sure this is large enough so the q value 
                                                            // multiplied by it is enough to recover balance
                                                            // when the ball gets a good speed up

        float timer = 0;                                    // keep track of how long the ball is balanced
        float maxBalanceTime = 9;                           // record time ball is kept balanced
        ...
        void Start()
        {
            ...
            Time.timeScale = 5.0f;                          // this causes the scene to run faster
        }
        ...
        // normalizes output values between 0 and 1 while also adding up to 1
        // this works well for percentage based algorithms
        // often used on output layer
        ...
        private void FixedUpdate()
        {
            ...
            List<double> states = new List<double>(); // rotx, ballz, velx //
            ...
            qs = SoftMax(ann.CalcOutput(states));  // tiltR, tiltL //
            double maxQ = qs.Max();
            ...
            if (ball.GetComponent<BallState>().dropped)
            {
                // this is the Q learning portion //
                for(int i= replayMemory.Count -1; i>= 0; i--)
                {
                    ...
                    double feedback;
                    // if we dropped the ball then we can't attribute anything prior to this round
